...
class IndividualclubRegExport implements FromView,WithHeadings, WithEvents
{
    /**
    ...
    */
    protected $categoriesclub;
    protected $filter;
    public function __construct($categoriesclub,$filter)
    {
        $this->categoriesclub = $categoriesclub;
        $this->filter = $filter;
    }
    ...
    public function view(): View
    {
            ...
            if ($this->categoriesclub) {
                foreach ($this->categoriesclub as $key => $values) {
                    if ($key == "Gender1") {
                        if($values!=0){
       ...
       return view('admin.reports.clubteam.individualClubRegFilter', [
        'clubregistations'=>  $clubregistations,
        'filter'=>  $this->filter
