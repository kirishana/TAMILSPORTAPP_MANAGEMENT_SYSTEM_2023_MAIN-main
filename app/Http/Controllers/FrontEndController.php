...
// --------------------------------------------------------------------------------------
     ...
     * @return View
     */
    public function getClub_Register()
    {
        // Show the page
        return view('club_register');
    }
     ...
     * @return Redirect
     */
    public function postClub_Register(UserRequest $request)
    {
        $activate = $this->user_activation; //make it false if you don't want to activate user automatically it is declared above as global variable

        try {
            // Register the user
            $user = Sentinel::club_register($request->only(['first_name', 'last_name', 'email', 'password', 'gender']), $activate);
     ...
     *
     */
    public function getClub_Activate($userId, $activationCode)
    {
        // Is the user logged in?
        if (Sentinel::check()) {
            return Redirect::route('my-account');
