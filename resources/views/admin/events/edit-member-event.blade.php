        ...
        <div class="panel panel-primary ">
            <div class="panel-heading">
                <h4 class="panel-title">
                    ...
                    {{ $reg->organization->name }}'s Events<div style="float:right">
...
@endif
                                ...
                                <div class="col-md-6 right-col">
                                    ...
                                    ?>
                                            ...
                                            @if($gender->gender_id==$userGender)
@if($gender->status!=10)
                                            <input type="checkbox" name="events2[]" id="events" class="live" data-field="{{$reg->organization->athelaticsetting->field_events}}" data-track="{{$reg->organization->athelaticsetting->track_events}}" data-field="{{$reg->organization->athelaticsetting->field_events}}" data-id="{{ $event->mainEvent->event_category_id }}" data-org="<?php echo $reg->organization->athelaticsetting->total_events ?>" value="{{ $event->id }}" {{ in_array($event->id , $events)? "checked":"" }}>{{$event->mainEvent->name}} - @php echo Carbon\Carbon::parse($event->date)->format('d.m.Y'); @endphp
                                            <br>
@endif
                                    ...
                                    if ($exp[0] == $age) {
                                        ...
                                        @if($gender->gender_id==$userGender)
@if($gender->status!=10)
                                        <input type="checkbox" name="events2[]" class="live" data-field="{{$reg->organization->athelaticsetting->field_events}}" data-track="{{$reg->organization->athelaticsetting->track_events}}" data-field="{{$reg->organization->athelaticsetting->field_events}}" data-id="{{ $event->mainEvent->event_category_id }}" data-org="<?php echo $reg->organization->athelaticsetting->total_events ?>" id="events" value="{{ $event->id }}" {{ in_array($event->id , $events)? "checked":"" }}>{{$event->mainEvent->name}} - @php echo Carbon\Carbon::parse($event->date)->format('d.m.Y'); @endphp
                                        <br>
    ...
    return false;
      ...
      $(document).ready(function($) {
        ...
        var org1 = $checkboxes.attr('data-org');
         var track=$checkboxes.attr('data-track');
        var field=$checkboxes.attr('data-field');
...
else if(track==$achecked){
 $("input[name='events[]']").not(':checked').attr("disabled", "disabled");
}
else if(field==$bchecked){
$("input[name='events2[]']").not(':checked').attr("disabled", "disabled");
}
...
$checkboxes.change(function() {
    ...
    $tr = $("input[name='events[]']").filter(':checked').length;
    $fl = $("input[name='events2[]']").filter(':checked').length;
if($tr==0 && $fl==0){
