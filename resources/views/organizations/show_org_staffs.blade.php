...
<section class="content paddingleft_right15">
        ...
        <div class="panel panel-primary ">
            <div class="panel-heading">
                <h4 class="panel-title">
                   ...
                   {{ __('staffs.staffs_lists') }}
          @if (Auth::guard('web')->user()->can('Create-staff2'))

                    <div style="float:right">
                        <a href="org/user/create" style="color:white"><i class="material-icons  leftsize">add_circle</i>
                                               {{ __('staffs.add_new') }}
</a>
                    </div>
                    @endif
            ...
            <div class="panel-body">
                <div class="row">
                    ...
                    <div class="col-md-2" style="display:flex; justify-content:flex-end;">
                        <a href="/export/staffId" target="_blank" title="Export Staff ID Card" class="btn btn-success"> 
                            <img style="margin-top:0px" src="{{ asset('assets/images/icons/pdf.png') }}"> &nbsp;   {{ __('staffs.export_id_card') }}
    ...
    <a id="btn15"  style="cursor:pointer;margin-right: 5px;"><img src="{{asset('assets/images/print.png')}}" style="float: right;"class="img-responsive" /> </a>
    <a href="/org/staff-list/export" target="_blank"> <img src="{{asset('assets/images/pdf.png')}}" style="float: right;margin-right:5px;" class="img-responsive" /></a>
    <a href="/org/staff-list/excel"> <img src="{{asset('assets/images/excel.png')}}"  style="float: right;margin-right:5px;"class="img-responsive" /></a>
</div>
                    </div>
                </div>
                <br>
                ...
                @if (count($users) > 0)
    <div class="row example" id="usr" style="overflow-x:scroll">
    <table class="table table-striped  table-bordered table-hover orgstaff" id="export">
        <thead style="background-color: #515763;  color: white;text-transform:uppercase;">
            <tr style="text-align: center">
                <th class="sorting" data-sorting_type="asc" data-column_name="first_name" style="cursor: pointer"><span style="float:left;" id="first_name_icon"><i class="fas fa-arrows-alt-v"></i></span>                   {{ __('staffs.f_name') }}
</th>
                ...
                <th class="phone">{{ __('dashboard.phone_no') }}</th>
                <th class="age">{{ __('staffs.role') }}</th>
                <th class="regDate">{{ __('dashboard.reg_date') }}</th>
                @if(Auth::user()->organization)
                @if(Auth::user()->organization->orgsetting)
                @if(Auth::user()->organization->orgsetting->active==1)
                <th class="regDate">{{ __('staffs.sil_mem') }}</th>
                @endif
                @endif
                @endif
                @if(Auth::user()->hasRole(4))
                @else
                <th class="phone" style="width:8%">{{ __('dashboard.actions') }}</th>
                @endif

            </tr>
        </thead>
        <tbody>
    @include('organizations.show_org_staffs_table')
    </tbody>
    </table>
...
@endif
    ...
    @include('organizations.show_org_staff_print')
</div>
...
$.ajax({
    type: 'POST',
    url: "/new-user/org-staff/approve/" + id,
    data: {
        "_token": "{{ csrf_token() }}",
        'id': id
    },
...
if (remark.trim().length != 0) {
    $.ajax({
        type: 'POST',
        url: "/new-user/org-staff/decline/" + id,
        data: {
            "_token": "{{ csrf_token() }}",
            'id': id
        },
...
function clear_icon(){
        ...
        function fetch_data(page,sort_type,sort_by,query2) {
            console.log(page);
            $.ajax({
                url: '/organization-staffs?page='+page,
                data:{
                    query2:query2,
                    'sorttypeStaff':sort_type,
                    'sortbyStaff':sort_by,
                }
            }).done(function (users) {
                $('tbody').html(users);
