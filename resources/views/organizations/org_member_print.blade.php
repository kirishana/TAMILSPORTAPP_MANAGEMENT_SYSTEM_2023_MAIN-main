...
<div  class="memberPrint" id="printmember">
...
border-collapse: collapse;">
  ...
  "class="age">No.</th> 
                ...
                <th style="width:4%;"class="age">Age</th> 
                  <th style="width:18%;"class="age">Club</th>     
                  @if(Auth::user()->organization)
                            @if(Auth::user()->organization->orgsetting)
                            @if(Auth::user()->organization->orgsetting->active==1) 
                  <th style="width:18%;"class="age">Sil-Member</th>      
                  @endif
                  @endif
                  @endif
            </tr>
        ...
        <tbody class="text-uppercase">
            @foreach($printusers as $key=>$user)
            <tr style="padding: 3px;
            width: 1%;
            text-align: left;">
            <td style="padding: 3px;
            width: 1%;
            text-align: left;width:15%">{{++$key}}</td>
            <td style="padding: 3px;
            width: 1%;
            text-align: left;width:15%">{{$user->first_name}}</td>
                ...
                <td style="padding: 3px;
                width: 1%;
                text-align: left;text-transform:lowercase;width:20%;">{{$user->email}}</td>
                <td style="padding: 3px;
                width: 1%;
                ...
                <td style="padding: 3px;
                width: 1%;
                text-align: left;text-transform:lowercase;width:20%;">{{$user->club->club_name}}</td>
                  @if(Auth::user()->organization)
                            @if(Auth::user()->organization->orgsetting)
                            @if(Auth::user()->organization->orgsetting->active==1)
                            <td style="width:18%;text-transform:capitalized;">{{$user->member_or_not==1 ? 'Yes' : 'No'}}</td>
                            @endif
                            @endif
                            @endif
            </tr>
